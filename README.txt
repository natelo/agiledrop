...
b) If set on "Pages"         
- It would require manual editing if desirable outcome is "events/" & node-X (event viewing page)     
- It would only be visible on "events/" and any other desirable page (i.e. root|index/)  

2) Extending/Patching     
a) Patch can be applied that introduces OR argument for "configure block" visibility so configuration can be used in shape of "ContentType AND|OR Pages" filtering   
Patch: https://www.drupal.org/project/drupal/issues/923934#comment-12401360   
Downside: It breaks compatibility thus introduces issues with future upgrades  
b) Extending functionality via existing plugins..      
i.e. https://www.drupal.org/project/block_visibility_conditions    
Downside: Creates dependency on external source as well as doesn't solve the "understanding/know how" aspect of things
...
A push in right direction (documentation/tutorial link) would probably render the issue obsolete quite fast.. 
